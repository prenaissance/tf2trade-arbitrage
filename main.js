...
const keyprice = 66; //for now constant
//const bptfkey = 59.11;
    ...
    hashtftrade.forEach((v, key) => {
        if (hashprices.has(key))
            prices.push([hashprices.get(key).name, key, v, hashprices.get(key).sell, hashprices.get(key).buy,
                parseFloat((v * 0.875 - (hashprices.get(key).sell.keys * bptfkey + hashprices.get(key).sell.metal)).toFixed(2)), //8% fee
                ((v * 0.875 - (hashprices.get(key).sell.keys * bptfkey + hashprices.get(key).sell.metal)) /
                    v * 100).toFixed(2),
                parseFloat(((hashprices.get(key).buy.keys * bptfkey + hashprices.get(key).buy.metal) - v).toFixed(2)),
                (((hashprices.get(key).buy.keys * bptfkey + hashprices.get(key).buy.metal) - v) / v * 100).toFixed(2)
            ]); //percentage gain buying
    });
    switch (myArgs[0]) {
        case "buy":
            prices.sort((a, b) => {
